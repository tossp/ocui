...
function readonlyCodeMirrorTheme(lineHeight: number): Extension {
  return EditorView.theme({
    ...
    '.cm-gutters': { backgroundColor: 'transparent', color: 'hsl(var(--text-500))', borderRight: '1px solid hsl(var(--border-100) / 0.35)' },
    '.cm-lineNumbers .cm-gutterElement': { minWidth: '2rem', padding: '0 0.75rem 0 1rem' },
    '.cm-activeLineGutter': { backgroundColor: 'hsl(var(--accent-main-100) / 0.08)', color: 'hsl(var(--accent-main-100))' },
    '.cm-activeLine': { backgroundColor: 'transparent' },
    '.cm-selectionBackground, &.cm-focused .cm-selectionBackground': { backgroundColor: 'rgb(255 255 255 / 0.15)' },
    '&.cm-focused > .cm-scroller > .cm-selectionLayer .cm-selectionBackground': { backgroundColor: 'rgb(255 255 255 / 0.15)' },
